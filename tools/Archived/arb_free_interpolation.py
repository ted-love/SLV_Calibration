...
def arbitrage_free_interpolation(Data,vol_df,N):
    ...
    interest_rates = Data.r
    dividend_rates = Data.q
   ...
   # option_prices = option_prices.reshape(np.shape(IVs)[0],np.shape(IVs)[1])
    ...
    forward = np.expand_dims(np.array(f),1)
    imp_vol = np.expand_dims(np.array(imp_vol),1)
    moneyness = K/forward
    
    x = np.concatenate((moneyness,T),axis=1)
    y = T*imp_vol ** 2
   ...
   # print('kappa 1' ,kappa_ini)
    kappa_ini = np.linspace(moneyness.min(),moneyness.max(),N).reshape(N,1)
  #  print('kappa 2',kappa_ini)
    ...
    forw = S_0 * np.exp((r-q)*tau.flatten())
    call_prices = np.exp(-(r-q)*tau.flatten()) * forw * py_vollib_vectorized.vectorized_black_scholes_merton(flag.squeeze(), S_0/S_0, kappa.flatten(), tau.flatten(), r, imp_vol_interpolated.flatten(), q, return_as='numpy')
    call_prices = call_prices.reshape(np.size(kappa_ini),np.size(tau_ini))
   # print(kappa.flatten())
   # print(tau.flatten())
  #  print(imp_vol_interpolated)
  #  print('t',tau)
    print(py_vollib_vectorized.vectorized_black_scholes_merton(flag.squeeze(), S_0/S_0, kappa.flatten(), tau.flatten(), 0, imp_vol_interpolated.flatten(), 0, return_as='numpy'))
